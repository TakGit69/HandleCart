...
const handleCart = (data, event) => {
    ...
    const productIndex = cart.findIndex(product => product.id === Number(productId));
    ...
    if (event.target.dataset.plus) {
      ...
      if (productIndex < 0) {
        cart.push({...foundedProduct, quantity: 1})
      } else {
        cart[productIndex].quantity += 1;
      }
    }
...
const plusMinusButtonsManager = () => {
  ...
  cartUl.addEventListener("click", (event) => {
    if (event.target.dataset.plus || event.target.dataset.minus) {
      handleCart(products, cart, event);
      createHTML(cart);
    }
  })  
}
const addToCartButtonManager = () => {
